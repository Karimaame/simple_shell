...
char **custom_strtow(char *str, char *d)
{
    ...
    for (i = 0; str[i] != '\0'; i++)
    {
        if (!is_custom_delim(str[i], d) && (is_custom_delim(str[i + 1], d) || !str[i + 1]))
            numwords++;
    }
    ...
    {
        while (is_custom_delim(str[i], d))
            i++;

        k = 0;

        while (!is_custom_delim(str[i + k], d) && str[i + k])
            k++;
...
/**
 * custom_strtow2 - splits a string into words based on a single delimiter character.
 * @str: the input string
